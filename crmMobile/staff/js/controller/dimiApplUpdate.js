 ...
 * 编辑离职流程
 */
app.controller('dimiApplUpdateCtrl', function($scope, $sessionStorage,
    $ionicPopup, $stateParams, $timeout, $state, $filter, comApi,
    ionicDatePicker) {
 ...
 // 取得下属人员列表(过滤掉已经申请的人员 10:未申请 60:已取消 20:未通过)
    ...
    // 取得入职流程信息
    comApi
        .post("dimiAppl/getDimiApplInfoForUpdatePageM", {
          dimiApplNum : $stateParams.dimiApplNum,
          updtTm : $stateParams.updtTm
        },
            function(data) {
              $scope.dimiInfo = data.dimiApplInfo;
              if(comApi.isNotNullAndUndefined(data.dimiApplInfo.jobLeaverNm)) {
                $scope.jobLeaverNmArr = angular.copy(data.dimiApplInfo.jobLeaverNm.split(','));
              } else {
                $scope.jobLeaverNmArr = [];
              }
              /*var leavers = data.dimiApplInfo.jobLeaver.split(",");
              for (var i = 0; i < $scope.undStfList.length; i++) {
                if (data.dimiApplInfo.jobLeaver
                    .indexOf($scope.undStfList[i].key) >= 0) {
                  $scope.undStfList[i].isCheck = true;
                }
              }*/
              $scope.selectedDate = new Date($scope.dimiInfo.dimiDt);
            });
  };
  ...
  $scope.goFrontPage = function() {
    ...
    confirmPopup.then(function(res) {
      if (res) {
        $state.go("dimiApplInfo", {
          applNum : $stateParams.dimiApplNum
        });
      }
    });
  };
