...
app
    .controller(
        ...
        function($scope, $sessionStorage, $ionicPopup, $ionicScrollDelegate,
            $timeout, $state, $window, $filter, comApi, ionicDatePicker) {
          ...
          $scope.init = function() {
            ...
            comApi.getSelectBoxDic("C003", 0).then(function(data) {
              $scope.dicList = data;
              $scope.entryInfo.sex = "2";
              $scope.entryInfo.sexDis = "女";
...
//                if($scope.chnlList[i].subStoreList.length==0){
//                  $scope.chnlList.splice(i,1);
//                }
//              }
//              $scope.sss.leng = "width:"+100/$scope.chnlList.length+"%"
         ...
         // 取得职位列表,门店列表
          ...
          $scope.positionSelect = function() {
            if($scope.selectedPos.stfNum){
                $scope.entryInfo.stfPos = $scope.selectedPos.stfNum;
            }else{
                //$scope.entryInfo.stfPos = $scope.positionList[0].posNum;
                $scope.entryInfo.stfPos = '';
            }
            $scope.posLvlListShow = false;
       ...
       // 选定职位
         ...
         // 直属主管
          ...
          $scope.saveEntryAppl = function() {
            ...
            comApi.post("entryAppl/saveEntryApplInsert", $scope.entryInfo,
                function(data) {
                  // 弹出提示消息
          ...
          $scope.submitEntryAppl = function() {
            ...
            comApi.post("entryAppl/submitEntryApplInsert", $scope.entryInfo,
                function(data) {
                  // 弹出提示消息
                  comApi.showMessage("success", "msg.common.10001", 3000);
                  // 3秒后跳转
