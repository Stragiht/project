/*
 *日常销售报表提交明细頁面的Controller 
 *--王健  2016-3-30 
 *引入的  $scope NgTableParams comApi $timeout $filter
 */
app.controller('baSaleReportDtlController', function($scope, NgTableParams,
    comApi, $timeout, $filter) {
  var self = this;
  var keyAll = "0000";
  var all = {
    key : keyAll,
  ...
  init();
  // 设定职位的下拉框
  $scope.posList = comApi.getSelectBoxJob(2);
  // 设定职位的默认值
  ...
  $scope.selectBaSaleReportDtl = function() {
    ...
    init();
    // 检索BA提交销售报表概况
    comApi.post("saleData/selectBaSaleReportDtl", $scope.cond,
        function(data) {
            // 设置检索结果
            setResult(data.avgAmt, data.maxAmt, data.minAmt, data.totalAmt,
                data.baSaleReportDtlList.length, data.baSaleReportDtlList);
            if (data.maxCount == data.baSaleReportDtlList.length) {
  ...
  $scope.exportCsv = function() {
    ...
    // 取得要导出的数据
    comApi.post("saleData/exportBaSaleReportDtl", $scope.cond,
        function(data) {
          $scope.getArray = data;
          // 设定导出文件名
          $scope.exportFileNm = "日常销售报表提交明细_"
