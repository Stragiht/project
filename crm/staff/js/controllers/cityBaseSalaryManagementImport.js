app.controller('CsdxjsglImportCtrl', ['$filter','$location','toaster','$scope','comApi','$state','Upload','$timeout', function($filter,$location,toaster,$scope,comApi,$state,Upload,$timeout) {
    ...
    //抓取上传控件在状态改变时,执行1的函数事件
      angular.element(document.querySelector('#fileInputMemBat')).on('change', handleFileSelect);
    ...
    $scope.importFile = function(){
      ...
      Upload.upload({
          url : '/CrmWeb/api/cbsbm/insertImportFile',
          data : {
          ...
          if(resp && resp.data && resp.data.meta && resp.data.meta != null){
              ...
              if(result.meta.success == true){
                  ...
                  setTimeout(function() {
                      $state.go("app.member.cityBaseSalaryManagement");}, "1000");
              }else if(result.data && result.data != null && result.data != ""){
                  $scope.csvFileName = "城市底薪基数excel导入错误日志.txt";
                  var errorMsg = [];
                  ...
                  $timeout(function(){angular.element(document.querySelector("#outCityBaseSalaryExcelImportErrorMsg")).triggerHandler("click");}, 1000);
              }else{
    ...
    $scope.models=function(){
      ...
      aLink.download = "城市底薪信息";
      aLink.href = $filter("imgpath")('model/城市底薪信息.xls');
