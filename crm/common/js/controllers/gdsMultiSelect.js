app.controller("gdsMultiSelectController",['$scope', '$modal', 'NgTableParams',
	...
	$scope.ok = function() {
		...
		//	var gdsData = $scope.list;
		//	//验证复选框是否选中
		//	for(var i =0; i<gdsData.length;i++){
		//		if(typeof($scope.checkboxes.items[gdsData[i].gdsSpecDtlNum]) != "undefined"){
		//			if($scope.checkboxes.items[gdsData[i].gdsSpecDtlNum]){
		...
		if(typeof($scope.list) != "undefined") {
			angular.forEach($scope.list, function (item) {
				if ($scope.checkboxes.items[item.gdsSpecDtlNum] == true) {
					datas.push(item);
				}
			});
		}
	...
	function initCheckBox() {
		...
		$scope.checkAll = function() {
			...
			angular
				...
				function(item) {
					...
					$scope.checkboxes.items[item.gdsSpecDtlNum] = $scope.checkboxes.checked;
				});
		};
		// 單選 的事件
		$scope.checkItem = function() {
			...
			angular
				...
				$scope.list,
				function(item) {
					checked += ($scope.checkboxes.items[item.gdsSpecDtlNum]) || 0;
					unchecked += (!$scope.checkboxes.items[item.gdsSpecDtlNum]) || 0;
				});
