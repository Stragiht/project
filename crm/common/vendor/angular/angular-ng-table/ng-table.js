        ...
        function($scope, NgTableParams, $timeout, $parse, $compile, $attrs, $element, ngTableColumn, ngTableEventsChannel) {
            ...
            this.compileDirectiveTemplates = function () {
                if (!$element.hasClass('ng-table')) {
                    ...
                    if (!theadFound) {
                        //张玉良修改. table 支持内滚动. ng-table 添加到div上才行
                        headerTemplate = angular.element(document.createElement('thead')).attr('ng-include', 'templates.header');
                        var divTable=$element.find("table");
                        divTable.prepend(headerTemplate);
                        //$element.prepend(headerTemplate);
                    }

                    $element.css("marginBottom","10px");

                    $element.niceScroll({
                        cursorcolor: "#D3D3D3",//#CC0071 光标颜色
                   ...
                   // $element.getNiceScroll(0).doScrollLeft(500, 300);
...
(function(){
    ...
    function ngTableSorterRowController($scope){
        ...
        $scope.sortBy = sortBy;
        $scope.xiugaikuandu = "width:300px;";
...
angular.module('ngTable').run(['$templateCache', function ($templateCache) {
	...
	$templateCache.put('ng-table/sorterRow.html', '<tr class="ng-table-sort-header"> <th title="{{$column.headerTitle(this)}}" ng-repeat="$column in $columns" ng-class="{ \'sortable\': $column.sortable(this), \'sort-asc\': params.sorting()[$column.sortable(this)]==\'asc\', \'sort-desc\': params.sorting()[$column.sortable(this)]==\'desc\' }" ng-click="sortBy($column, $event)" ng-if="$column.show(this)" ng-init="template=$column.headerTemplateURL(this)" class="header {{$column.class(this)}}"> <div ng-if="!template" class="ng-table-header" style="width: {{$column.widthPy(this)}}" ng-class="{\'sort-indicator\': params.settings().sortingIndicator==\'div\'}"> <span ng-bind="$column.title(this)" ng-class="{\'sort-indicator\': params.settings().sortingIndicator==\'span\'}"></span> </div> <div ng-if="template" style="width: {{$column.widthPy(this)}}" ng-include="template"></div> </th> </tr> ');
}]);
