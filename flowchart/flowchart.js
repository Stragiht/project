...
function setProperty(diagram) {
    ...
    diagram.selection.each(function (node) {
        ...
        if (node instanceof go.Node) {  // ignore any selected Links and simple Parts
            ...
            //添加与操作者相关的数据的回填 吕晓亮 2016-02-19
            //选择角色
            $("#F_nodeSelectedRoles").val(NodesData.selectedRoles);
            //操作者从属关系
            switch (NodesData.operatorBelongRelation) {
            	case BPMEnumValue.BpmDefProcess.OPERATOR_BELONG_RELATION.NO:
            		$('input[name=F_nodeOperatorBelongRelation][value="NO"]')[0].checked = true;
            		break;
            	case BPMEnumValue.BpmDefProcess.OPERATOR_BELONG_RELATION.UP:
            		$('input[name=F_nodeOperatorBelongRelation][value="UP"]')[0].checked = true;
            		break;
            	case BPMEnumValue.BpmDefProcess.OPERATOR_BELONG_RELATION.DOWN:
            		$('input[name=F_nodeOperatorBelongRelation][value="DOWN"]')[0].checked = true;
            		break;
            	case BPMEnumValue.BpmDefProcess.OPERATOR_BELONG_RELATION.SAME:
            		$('input[name=F_nodeOperatorBelongRelation][value="SAME"]')[0].checked = true;
            		break;
            }
            //组织从属关系
            switch (NodesData.organizationBelongRelation) {
	        	case BPMEnumValue.BpmDefProcess.ORGANIZATION_BELONG_RELATION.NO:
	        		$('input[name=F_nodeOrganizationBelongRelation][value="NO"]')[0].checked = true;
	        		break;
	        	case BPMEnumValue.BpmDefProcess.ORGANIZATION_BELONG_RELATION.UP:
	        		$('input[name=F_nodeOrganizationBelongRelation][value="UP"]')[0].checked = true;
	        		break;
	        	case BPMEnumValue.BpmDefProcess.ORGANIZATION_BELONG_RELATION.DOWN:
	        		$('input[name=F_nodeOrganizationBelongRelation][value="DOWN"]')[0].checked = true;
	        		break;      
            }
            //从属关系源节点编号
            $("#F_nodeRelationSrcPnCode").val(NodesData.relationSrcPnCode);           
            //是否允许无操作者
            switch (NodesData.allowOperatorNone) {
	        	case BPMEnumValue.BpmDefProcess.ALLOW_OPERATOR_NONE.NO:
	        		$('input[name=F_nodeAllowOperatorNone][value="NO"]')[0].checked = true;
	        		break;
	        	case BPMEnumValue.BpmDefProcess.ALLOW_OPERATOR_NONE.YES:
	        		$('input[name=F_nodeAllowOperatorNone][value="YES"]')[0].checked = true;
	        		break;    
	        }
...
function ClearState() {
    ...
    //添加与操作者相关的数据清除 吕晓亮 2016-02-19
    //选择角色
    $("#F_nodeSelectedRoles").val("");
    //操作者从属关系
    $('input[name=F_nodeOperatorBelongRelation][value="NO"]')[0].checked = true;
    //组织从属关系
    $('input[name=F_nodeOrganizationBelongRelation][value="NO"]')[0].checked = true;
    //从属关系源节点编号
    $("#F_nodeRelationSrcPnCode").val("");           
    //是否允许无操作者 
    $('input[name=F_nodeAllowOperatorNone][value="YES"]')[0].checked = true;
    ...
    if (selectLink != null) {
        ...
        diagram.selection.each(function (node) {
            if (node instanceof go.Node) {  // ignore any selected Links and simple Parts
                ...
                //添加与操作者相关的数据UI-->BpmDefProcess 吕晓亮 2016-02-19
                //选择角色
                BpmDefProcess.selectedRoles = $("#F_nodeSelectedRoles").val();
                //操作者从属关系
                _operatorBelongRelation = $("input[name='F_nodeOperatorBelongRelation']:checked").attr('value');
                BpmDefProcess.operatorBelongRelation = BPMEnumValue.BpmDefProcess.OPERATOR_BELONG_RELATION[_operatorBelongRelation];
                //组织从属关系
                _organizationBelongRelation = $("input[name='F_nodeOrganizationBelongRelation']:checked").attr('value');
                BpmDefProcess.organizationBelongRelation = BPMEnumValue.BpmDefProcess.ORGANIZATION_BELONG_RELATION[_organizationBelongRelation];
                //从属关系源节点编号
                BpmDefProcess.relationSrcPnCode = $("#F_nodeRelationSrcPnCode").val();           
                //是否允许无操作者
                _allowOperatorNone = $("input[name='F_nodeAllowOperatorNone']:checked").attr('value');
                BpmDefProcess.allowOperatorNone = BPMEnumValue.BpmDefProcess.ALLOW_OPERATOR_NONE[_allowOperatorNone];
                ...
                if(event.eventType=="ProcessNodeBeforeStartEvent"){
                    ...
                    $(newNode).find('.nextStep').bind('click',function(){
                        ...
                        $.each(datalist,function(datalstindex,datalistitem){
                            ...
                            if(_key==null||_key==""||_key==undefined||_value==null||_value==""||_value==undefined) {
                                alert("页面数据获取错误！")
                            }
                            else{
                                //提交数据
                                BpmRestController.bpm.putProcessData(_event.businessProcessOid,_key,_value,function(){
...
//获取当前角色集合
function GetAllRole(){
	BpmRestController.bpm.getAllBpmRole(function(data ){
        if(data){
        	var _roleString = "";
        	$('#roleList').val("");
        	...
        	$.each(data, function(index, node){
        		if(_roleString != ""){
        			_roleString = _roleString + "," + node.roleId;
        		}else{
        			_roleString = node.roleId;
        		}
        	})
            $('#roleList').val(_roleString);
        }
    })
}